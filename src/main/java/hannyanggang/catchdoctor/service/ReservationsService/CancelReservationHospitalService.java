...
@Service
public class CancelReservationHospitalService {
    private final ReservationsRepository reservationsRepository;
    private final HospitalRepository hospitalRepository;

    @Transactional
    public ResponseEntity<?> cancelReservation(ReservationsDTO reservationsDTO, String hospitalId) {
        ...
        // 예약 상태가 예약 확정인 경우
        if(Objects.equals(status, "예약신청")){
            reservations.setStatus("예약취소");
        } // 예약 상태가 예약 취소인 경우
        else if (Objects.equals(status, "예약취소")) {
            return ResponseEntity.ok("이미 예약 취소된 예약건 입니다. 현재 상태: " + status);
        } // 예약 상태가 예약 확정인 경우
        else if (Objects.equals(status, "예약확정")) {
            return ResponseEntity.ok("이미 예약 확정된 예약입니다. 현재 상태: " + status);
        } // 예약 상태가 진료 완료인 경우
